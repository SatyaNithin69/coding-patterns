...
function find_k_largest_pairs(nums1, nums2, k) {
  ...
  minHeap.forEach(element => {
    res.push([nums1[element[1]], nums2[element[2]]]);
  });
  return res;
}
...
/**
Time complexity #
Since, at most, we’ll be going through all the elements of both arrays and we will add/remove one element in the heap in each step, the time complexity 
of the above algorithm will be O(N*M*logK)O(N∗M∗logK) where ‘N’ and ‘M’ are the total number of elements in both arrays, respectively.
